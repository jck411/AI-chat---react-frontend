...
class PushAudioOutputStreamCallback(speechsdk.audio.PushAudioOutputStreamCallback):
    """
    A callback that Azure TTS will use to push PCM data into an asyncio Queue.
    """
...
async def azure_text_to_speech_processor(phrase_queue: Queue,
                                         audio_queue: Queue,
                                         stop_event: Event):
    """
    Continuously read text from phrase_queue, convert to speech with Azure TTS,
    and push PCM data into audio_queue. Stops if stop_event is set.
    """
    try:
