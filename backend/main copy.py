            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        raise
                    conditional_print(f"Retry {attempt+1}/{max_retries} for {func.__name__} due to: {str(e)}")
                    await asyncio.sleep(delay * (attempt + 1))
        return wrapper
    return decorator

def fallback(default=None):
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            try:
                ...
                conditional_print(f"Fallback activated for {func.__name__}: {str(e)}")
                return default
        return wrapper
    return decorator
...
def safe_initialize_client():
    ...
    try:
        ...
        if API_HOST == "openai":
            ...
            raise ValueError(f"Unsupported API host: {API_HOST}")

    except Exception as e:
        conditional_print(f"Critical initialization error: {str(e)}")
        # Fallback to OpenAI if configured provider fails
        conditional_print("Attempting OpenAI fallback...")
        client = openai.AsyncOpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        DEPLOYMENT_NAME = "gpt-3.5-turbo"  # Default fallback model

# Initialize client with error handling
try:
    safe_initialize_client()
except Exception as e:
    conditional_print(f"Failed to initialize API client: {str(e)}")
    raise SystemExit("Critical initialization failure")
...
# =====================================================================================
# Enhanced Audio Components with Error Recovery
# =====================================================================================
class RobustAudioPlayer(AudioPlayer):
    async def safe_write_audio(self, data: bytes):
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise
                conditional_print(f"Audio write error, attempt {attempt+1}/3: {str(e)}")
                self.restart_stream()
...
audio_player = RobustAudioPlayer(pyaudio_instance)
...
# =====================================================================================
# Enhanced STT Service with Retries
# =====================================================================================
class ResilientSpeechRecognizer(ContinuousSpeechRecognizer):
    ...
    async def start_listening(self):
        try:
            if not self.is_listening:
                self.speech_recognizer.start_continuous_recognition()
                self.is_listening = True
                conditional_print("STT service started successfully")
        except Exception as e:
            self.is_listening = False
            conditional_print(f"STT start failed: {str(e)}")
            raise

    async def robust_get_speech(self):
        try:
            return self.speech_queue.get_nowait()
        except Exception as e:
            conditional_print(f"Speech queue error: {str(e)}")
            return None

stt_instance = ResilientSpeechRecognizer()

# =====================================================================================
# Enhanced TTS Services with Fallbacks
# =====================================================================================
@fallback(default=None)
async def safe_azure_tts(phrase: str, speech_config: speechsdk.SpeechConfig, audio_cfg: speechsdk.audio.AudioOutputConfig):
    """
    Safely synthesizes speech using Azure TTS with a fallback mechanism.

    Args:
        phrase (str): The SSML-formatted phrase to synthesize.
        speech_config (speechsdk.SpeechConfig): Azure speech configuration.
        audio_cfg (speechsdk.audio.AudioOutputConfig): Azure audio output configuration.

    Returns:
        speechsdk.Result: The result of the speech synthesis.
    """
    ...
    result = await asyncio.get_event_loop().run_in_executor(
        None, 
        lambda: synthesizer.speak_ssml_async(phrase).get()
    )
    return result

async def resilient_azure_tts_processor(phrase_queue: asyncio.Queue, audio_queue: asyncio.Queue, stop_event: asyncio.Event):
    """
    Continuously reads text from phrase_queue, converts it to speech using Azure TTS,
    and pushes PCM data into audio_queue. Implements retry logic for robustness.
    Stops early if stop_event is set.

    Args:
        phrase_queue (asyncio.Queue): Queue containing phrases to synthesize.
        audio_queue (asyncio.Queue): Queue to push synthesized audio data.
        stop_event (asyncio.Event): Event to signal stopping of the processor.
    """
    try:
        ...
        while True:
            ...
            for attempt in range(3):
                try:
                    result = await safe_azure_tts(ssml_phrase, speech_config, audio_cfg)
                    if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                        conditional_print("Azure TTS synthesis completed successfully.", "default")
                        break
                    elif attempt == 2:
                        raise RuntimeError("Azure TTS failed after 3 attempts")
                except Exception as e:
                    conditional_print(f"Azure TTS attempt {attempt+1}/3 failed: {str(e)}", "default")
                    await asyncio.sleep(0.5 * (attempt + 1))
            else:
                # If all attempts fail, signal the failure
                conditional_print("All Azure TTS attempts failed. Disabling TTS.", "default")
                CONFIG["GENERAL_TTS"]["TTS_ENABLED"] = False
                await audio_queue.put(None)
                return

    except Exception as e:
        conditional_print(f"Azure TTS unrecoverable error: {str(e)}", "default")
        CONFIG["GENERAL_TTS"]["TTS_ENABLED"] = False
        await audio_queue.put(None)
...
# =====================================================================================
# Enhanced API Tools with Error Handling
# =====================================================================================
@retry(max_retries=3, delay=1.0)
async def resilient_fetch_weather(lat=28.5383, lon=-81.3792, exclude="minutely", units="metric", lang="en"):
    try:
        return fetch_weather(lat, lon, exclude, units, lang)
    except requests.exceptions.RequestException as e:
        conditional_print(f"Weather API error: {str(e)}")
...
async def process_streams(phrase_queue: asyncio.Queue, audio_queue: asyncio.Queue, stop_event: asyncio.Event):
    ...
    try:
        ...
        if provider == "azure":
            tts_processor = resilient_azure_tts_processor
